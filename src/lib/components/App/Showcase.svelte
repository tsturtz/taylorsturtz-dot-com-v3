<script lang="ts">
	...
	const toejamAndEarlJamOut: Project = {
		...
		description: `A for-fun recreation of the <em>Toe Jam and Earl™</em> mini-game, <em>Jam Out</em> in the browser.`
	};
	const todoForYou: Project = {
		...
		description: `Ah yes, the <em>to-do</em> app. This one is feature-rich and polished though 😉.`
	};
	// macOS
	...
	const zoombot: Project = {
		...
		stack: ['C++ (Arduino)'],
		description: `Zoombot is an IOT conference room bot for allowing remote participants to be more engaged in meetings. Took second place in the <em>Blizzard Hackathon, 2022</em>! My role was programming an arduino microcontroller to poll a server for incoming events, queue them up, and process them.`,
		award: { color: 'silver', title: 'Blizzard Hackathon, 2022 - 2nd Place' }
	};
	// mobile
	...
	$: filteredProjects = selectedProjectType
...
<div id="showcase-content" class="centered-content">
	...
	<div id="showcase-filter">
		...
		{#each filterOptions as filterOption, idx}
			<Button
				class="text"
				selected={selectedProjectType === filterOption}
				on:click={() => filter(filterOption)}
			>
				<div class="filter-button">
					<svelte:component this={projectIconMap[filterOption]} --size="16px" />
					<span class="filter-text"
						>{projectFiendlyNameMap[filterOption]} ({projects.filter(
							(project) => project.type === filterOption
						).length})</span
					>
				</div>
			</Button>
	...
	<div id="showcase-grid">
		{#each filteredProjects as project}
			...
			>
				<svelte:component this={projectIconMap[project.type]} slot="badge" />
				<svelte:fragment slot="award">
					{#if project.award}
						<span
							style={`color: ${awardColorMap[project.award.color]};`}
							title={project.award.title}
						>
							<AwardIcon --size="20px" />
						</span>
					{/if}
