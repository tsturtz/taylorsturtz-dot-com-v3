...
{#if maximized}
  <Lightbox imagePath={metaImageAnimated || metaImage} handleLightboxToggle={handleLightboxToggle} />
{/if}
<div class="accent-box showcase-box {maximized ? 'maximized' : ''}">
  <div
    class="meta"
    style="background-image: url({animateMetaImage && metaImageAnimated ? metaImageAnimated : metaImage});"
    on:mouseenter={() => { animateMetaImage = true }}
    on:mouseleave={() => { animateMetaImage = false }}
  >
  ...
  <div class="content">
    <div>
      ...
      <div class="description {descLines > descLinesLimit ? 'overflowing' : ''}">
        <p bind:this={desc} class="{descLines === 9 ? 'by1' : descLines === 10 ? 'by2' : descLines === 11 ? 'by3' : descLines === 12 ? 'by4' : ''}">{@html description}</p>
      </div>
      <br />
    </div>
    <div class="stack">
